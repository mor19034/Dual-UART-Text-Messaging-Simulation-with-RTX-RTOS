...
osMailQDef(mail_queue, 100, Mail); // Define a mail queue of 100 blocks
...
/*----------------------------------------------------------------------------
  Timer callback function. Wakes up Auto_Mess1_Thread
 *---------------------------------------------------------------------------*/
void callback(void const *param)
{
...
/*----------------------------------------------------------------------------
  Timer callback function. Wakes up Auto_Mess2_Thread
 *---------------------------------------------------------------------------*/
void callback1(void const *param)
{
 ...
 * UART1_Rx_Thread
 *
 * Waits for bytes from the UART1 ISR (signal 0x01 / Q_UART1) and hands each
 * one to the shared UART_Rx_Process() state machine for UART_1.
 *---------------------------------------------------------------------------*/
void UART1_Rx_Thread (void const *argument)
{
	UART_Context_Init(UART_1);
	...
	for (;;)
	{
		...
		UART_Rx_Process(UART_1, intKey1);
	}
}
 ...
 * UART2_Rx_Thread
 *
 * Waits for bytes from the UART2 ISR (signal 0x02 / Q_UART2) and hands each
 * one to the shared UART_Rx_Process() state machine for UART_2.
 *---------------------------------------------------------------------------*/
void UART2_Rx_Thread (void const *argument)
{
	UART_Context_Init(UART_2);
 ...
 * UART3_Rx_Thread
 *
 * Waits for bytes from the UART3 ISR (signal 0x02 / Q_UART3) and hands each
 * one to the shared UART_Rx_Process() state machine for UART_3.
 *---------------------------------------------------------------------------*/
void UART3_Rx_Thread (void const *argument)
{
	UART_Context_Init(UART_3);
...
/*----------------------------------------------------------------------------
 * Auto_Mess2_Thread - periodic "Hearbeat" message generator
 *---------------------------------------------------------------------------*/
void Auto_Mess2_Thread (void const *argument)
{
...
/*----------------------------------------------------------------------------
 * Auto_Mess1_Thread - periodic "AUTO" message generator
 *---------------------------------------------------------------------------*/
void Auto_Mess1_Thread (void const *argument)
{
	Recipient_ID current_out = Recipient_1;
...
int main (void)
{
	...
	SendText1((uint8_t*)"Welcome, user 1\n \n");
	SendText2((uint8_t*)"Welcome, user 2\n \n");
	SendText3((uint8_t*)"Welcome, user 3\n \n");
